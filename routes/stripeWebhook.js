...
// Stripe's optional Customer Portal "cancellation reason" feature reports
// each reason as one of these fixed codes — translated here so the admin
// alert reads naturally instead of showing raw snake_case.
const CANCELLATION_REASON_LABELS = {
  too_expensive: 'Πολύ ακριβό',
  missing_features: 'Λείπουν χαρακτηριστικά',
  switched_service: 'Πήγε αλλού',
  unused: 'Δεν το χρησιμοποιούσε',
  customer_service: 'Εξυπηρέτηση πελατών',
  low_quality: 'Χαμηλή ποιότητα',
  other: 'Άλλο',
};
...
// Refer-a-friend: if this person joined Discord via an invite created by
// an existing active subscriber, and that referral hasn't been rewarded
// yet, give the referrer a one-cycle discount on their own subscription.
// Never blocks the checkout flow — a lookup miss or Stripe error here
// just means no reward went out, not a failed signup.
if (discordUsername) {
  try {
    const referral = await findUnrewardedReferralByReferredUsername(discordUsername);
    if (referral) {
      const rewarded = await applyReferralReward(referral.referrerUsername);
      if (rewarded) {
        await markRewardApplied(referral.rowNumber);
        try {
          await sendChannelMessage(
            process.env.ADMIN_ALERT_CHANNEL_ID,
            `🎁 **Refer-a-friend reward**
            Ο/Η ${referral.referrerUsername} πήρε έκπτωση επόμενου μήνα επειδή έφερε τον/την ${discordUsername}.`
            );
        } catch (err) {
          console.error('Failed to send admin Discord alert for referral reward:', err.message);
        }
      }
    }
  } catch (err) {
