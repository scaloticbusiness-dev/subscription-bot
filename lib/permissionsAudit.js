...
// `match` is checked as a case-insensitive substring against the raw API
// channel name (always plain lowercase-with-dashes, no emoji). Add more
// entries here as we lock down intended behaviour for other channels —
// anything not listed still shows up in the snapshot, just unflagged.
const CHANNEL_RULES = [
  {
    match: 'wins',
    label: '#wins',
...
async function runPermissionsAudit() {
  ...
  for (const channel of textChannels) {
    ...
    const rule = CHANNEL_RULES.find((r) => channel.name.toLowerCase().includes(r.match));
    if (!rule) continue;
    ...
    if (rule.expectRoles) {
      for (const [roleName, expected] of Object.entries(rule.expectRoles)) {
        const found = roleOverwrites.find((r) => r.roleName === roleName);
        if (!found) {
