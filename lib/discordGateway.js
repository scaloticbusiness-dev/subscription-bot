...
const { ensureExpensesSheet, addExpense, getExpensesForMonth, formatMoney } = require('./expenses');
const { getAllToolRenewals } = require('./toolRenewals');
  ...
  const c = new Client({
    ...
    c.on('interactionCreate', async (interaction) => {
          try {
                  ...
                  } else if (interaction.commandName === 'log-expense') {
                            await handleLogExpenseCommand(interaction);
                  } else if (interaction.commandName === 'expense-dashboard') {
                            await handleExpenseDashboardCommand(interaction);
                  }
          } catch (err) {
...
async function registerSlashCommands(c) {
    await c.application.commands.set([
      ...
      {
              name: 'log-expense',
              description: 'Κατέγραψε ένα έκτακτο έξοδο (π.χ. διαφήμιση, freelancer, αγορά)',
              default_member_permissions: PermissionFlagsBits.ManageGuild.toString(),
              options: [
                { name: 'amount', description: 'Ποσό (π.χ. 49.90)', type: 10, required: true },
                { name: 'category', description: 'Κατηγορία (π.χ. Ads, Freelancers, Software)', type: 3, required: true },
                { name: 'description', description: 'Σύντομη περιγραφή (προαιρετικό)', type: 3, required: false },
                { name: 'currency', description: 'Νόμισμα (προεπιλογή EUR)', type: 3, required: false },
                      ],
      },
      {
              name: 'expense-dashboard',
              description: 'Δες τα συνολικά έξοδα του μήνα (συνδρομές + έκτακτα έξοδα)',
              default_member_permissions: PermissionFlagsBits.ManageGuild.toString(),
      },
        ]);
    console.log('Registered /faq, /announce-lesson, /transcribe, /search-transcripts, /poll, /send-replay, /migrate-to-yearly, /log-expense, and /expense-dashboard slash commands.');
}
/**
...
/**
 * Handles the /log-expense slash command — appends one row to the
 * "Expenses" sheet tab (lib/expenses.js). This is for one-off/manual
 * costs (ads, freelancers, one-time purchases); recurring tool
 * subscriptions have their own separate tracker (lib/toolRenewals.js,
 * via the Google Sheet directly — no slash command for those).
 */
async function handleLogExpenseCommand(interaction) {
  await interaction.deferReply({ ephemeral: true });

  const amount = interaction.options.getNumber('amount');
  const category = interaction.options.getString('category');
  const description = interaction.options.getString('description') || '';
  const currency = (interaction.options.getString('currency') || 'EUR').toUpperCase();
  ...
  try {
    ...
    await interaction.editReply({
      content: `✅ Καταγράφηκε έξοδο: ${formatMoney(amount, currency)} — ${category}${description ? ` (${description})` : ''}.`,
    });
    console.log(`Logged expense via /log-expense: ${formatMoney(amount, currency)} (${category}), by ${interaction.user.username}.`);
  } catch (err) {
    console.error('Failed to log expense:', err.message);
    await interaction.editReply({ content: `❌ Κάτι πήγε στραβά: ${err.message}` });
  }
}
/**
 * Handles the /expense-dashboard slash command — builds an on-demand
 * snapshot of the current month's expenses, combining:
 *   1. The monthly-equivalent run-rate of every active row in the "Tool
 *      Renewals" tab (lib/toolRenewals.js) — yearly costs are divided by
 *      12 so they're comparable to monthly ones.
 *   2. Manually-logged one-off expenses for this month so far
 *      (lib/expenses.js, via /log-expense).
 * Grouped and totaled per currency separately, since mixing currencies
 * into one sum would silently produce a wrong number.
 */
async function handleExpenseDashboardCommand(interaction) {
  await interaction.deferReply({ ephemeral: true });

  try {
    await ensureExpensesSheet();
    const [renewals, monthly] = await Promise.all([
      getAllToolRenewals(),
      getExpensesForMonth(0),
    ]);

    const recurringByCurrency = {};
    for (const row of renewals) {
      const cost = parseFloat(row.cost);
      if (!cost) continue;
      const currency = (row.currency || 'EUR').toUpperCase();
      const monthlyEquivalent = (row.billingCycle || '').toLowerCase() === 'yearly' ? cost / 12 : cost;
      recurringByCurrency[currency] = (recurringByCurrency[currency] || 0) + monthlyEquivalent;
    }

    const currencies = new Set([
      ...Object.keys(recurringByCurrency),
      ...Object.keys(monthly.totalsByCurrency),
    ]);

    if (currencies.size === 0) {
      await interaction.editReply({
        content: `💰 **Έξοδα — ${monthly.label}**\n\nΔεν υπάρχουν ακόμα καταγεγραμμένες συνδρομές (Tool Renewals) ή έκτακτα έξοδα (/log-expense) για σύγκριση.`,
      });
      return;
    }
    ...
    for (const currency of currencies) {
      const recurring = recurringByCurrency[currency] || 0;
      const manual = monthly.totalsByCurrency[currency] || 0;
      const total = recurring + manual;

      const categoryLines = Object.entries(monthly.byCategory[currency] || {})
        .sort((a, b) => b[1] - a[1])
        .map(([cat, amt]) => `   • ${cat}: ${formatMoney(amt, currency)}`)
        .join('\n');

      sections.push(
        [
          `**${currency}**`,
          `Επαναλαμβανόμενες συνδρομές (μηνιαίο ισοδύναμο): ${formatMoney(recurring, currency)}`,
          `Έκτακτα έξοδα αυτού του μήνα: ${formatMoney(manual, currency)}`,
          categoryLines ? categoryLines : null,
          `**Σύνολο: ${formatMoney(total, currency)}**`,
        ].filter(Boolean).join('\n')
      );
    }

    await interaction.editReply({
      content: `💰 **Έξοδα — ${monthly.label} (μέχρι σήμερα)**\n\n${sections.join('\n\n')}`,
    });
    console.log(`Sent /expense-dashboard snapshot for ${monthly.label} to ${interaction.user.username}.`);
  } catch (err) {
    console.error('Failed to build /expense-dashboard:', err.message);
    await interaction.editReply({ content: `❌ Κάτι πήγε στραβά: ${err.message}` });
  }
}
/**
