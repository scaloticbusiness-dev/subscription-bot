...
function buildClient() {
  ...
  c.once('ready', async () => {
    ...
    try {
      await registerFaqCommand(c);
    } catch (err) {
      console.error('Failed to register /faq slash command:', err.message);
    }
  });
  ...
  c.on('interactionCreate', async (interaction) => {
    try {
      ...
      console.error('Failed to handle /faq command:', err.message);
    }
  });
  c.on('error', (err) => {
...
/**
 * Registers the /faq global slash command. Uses `.set()` (bulk overwrite)
 * rather than `.create()` so re-running this on every startup never
 * creates duplicates — it just re-declares the same single command.
 * Global commands can take up to ~1 hour to first appear after creation
 * (a normal Discord limitation, not a bug); updates after that are fast.
 */
async function registerFaqCommand(c) {
  ...
  console.log('Registered /faq slash command.');
}
/**
