...
async function getAllLeads() {
  ...
  const dateIdx = headers.indexOf('Ημερομηνία');
    ...
    const channelStatusIdx = headers.indexOf('1 — Πού βρίσκεται το κανάλι');
    const budgetIdx = headers.indexOf('4 — Διαθέσιμο κεφάλαιο');
    const pageIdx = headers.indexOf('Σελίδα');
  ...
  const rows = res.data.values || [];
    return rows.map((row, idx) => ({
          ...
          channelStatus: channelStatusIdx >= 0 ? row[channelStatusIdx] || '' : '',
          budget: budgetIdx >= 0 ? row[budgetIdx] || '' : '',
