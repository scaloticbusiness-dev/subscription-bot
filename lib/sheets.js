...
// A Name | B Email | C Discord Username | D Date | E Renewal Date | F Status | G Plan | H Amount

const { google } = require('googleapis');

const RANGE_ALL = 'A2:H';
const HEADER_ROW = ['Name', 'Email', 'Discord Username', 'Date', 'Renewal Date', 'Status', 'Plan', 'Amount'];
 ...
 */
async function appendRow({ name, email, discordUsername, date, renewalDate, status, plan, amount }) {
  const sheets = await getClient();
  const spreadsheetId = process.env.GOOGLE_SHEET_ID;
  ...
  await sheets.spreadsheets.values.append({
    ...
    requestBody: {
      values: [[name, email, discordUsername, date, renewalDate, status, plan, amount]],
    },
  });
}
...
module.exports = { getAllRows, findRowByEmail, appendRow, updateRow, ensureHeaderRow };
