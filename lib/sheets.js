...
// W Privacy Policy Version | X Renewal Reminder Sent

const { google } = require('googleapis');

const RANGE_ALL = 'A2:X';
const HEADER_ROW = [
  'Name',
  'Email',
  'Discord Username',
...
const rows = res.data.values || [];
  return rows.map((row, idx) => ({
    ...
    tosVersion: row[20] || '',
    taxCountry: row[21] || '',
    privacyPolicyVersion: row[22] || '',
    renewalReminderSent: row[23] || '',
  }));
}
...
const columnMap = {
  ...
  tosVersion: 'U',
  taxCountry: 'V',
  privacyPolicyVersion: 'W',
  renewalReminderSent: 'X',
};
...
await sheets.spreadsheets.values.update({
  spreadsheetId,
  range: `${ARCHIVE_SHEET_NAME}!A1:X1`,
  valueInputOption: 'USER_ENTERED',
  requestBody: { values: [HEADER_ROW] },
});
...
const rows = res.data.values || [];
  const matchIndex = rows.findIndex((row) => (row[1] || '').toLowerCase() === (email || '').toLowerCase());
  if (matchIndex === -1) return null;
  const match = rows[matchIndex];

return {
  rowNumber: matchIndex + 2, // +2: header row + 0-indexed -> 1-indexed
  name: match[0] || '',
  email: match[1] || '',
  ...
  tosVersion: match[20] || '',
  taxCountry: match[21] || '',
  privacyPolicyVersion: match[22] || '',
};
}

/**
* Deletes a single row (1-based row number) from the Archive tab. Unlike
* deleteRow above (main tab, fixed MAIN_SHEET_GRID_ID), the Archive tab is
* created later at runtime (see ensureArchiveSheet), so its sheetId isn't
* known ahead of time — this looks it up first. Safe no-op if the Archive
* tab doesn't exist. Used by the GDPR deletion endpoint.
*/
async function deleteArchiveRow(rowNumber) {
  const sheets = await getClient();
  const spreadsheetId = process.env.GOOGLE_SHEET_ID;

const meta = await sheets.spreadsheets.get({ spreadsheetId });
  const archiveSheet = meta.data.sheets.find((s) => s.properties.title === ARCHIVE_SHEET_NAME);
  if (!archiveSheet) return; // nothing to delete, tab doesn't exist
